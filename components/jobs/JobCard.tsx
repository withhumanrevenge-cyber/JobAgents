...
export function JobCard({ match }: { match: Match }) {
  ...
  const cardClass = "bg-card border border-border rounded-lg p-4 hover:border-foreground/40 transition-colors flex flex-col h-full cursor-pointer"
...
function renderCardBody(match: Match, job: NonNullable<Match["job"]>) {
  return (
    <>
      ...
      <p className="text-sm font-medium text-foreground leading-snug mb-0.5 line-clamp-2">{job.title}</p>
      ...
      <div className="flex flex-wrap gap-1 mb-4">
        {job.tags.slice(0, 3).map((tag, i) => (
          <span key={i} className="bg-muted border border-border text-[10px] text-muted-foreground px-2 py-0.5 rounded">{tag}</span>
        ))}
      </div>

      <div className="mt-auto pt-3 border-t border-border/60 flex justify-between items-center">
        <span className="text-[10px] text-muted-foreground/60 uppercase tracking-wider">{SOURCE_LABEL[job.source] ?? job.source}</span>
        <Link href={`/jobs/${job.id}`} className="text-xs text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors">
