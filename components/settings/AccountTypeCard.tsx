    ...
    <div className="bg-card border border-border rounded-lg p-4 space-y-3">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">
          {accountType === "recruiter"
            ? <>Hiring as <span className="font-medium text-foreground/80">{companyName || "your company"}</span>.</>
      ...
      {accountType === "recruiter" ? (
        <button onClick={becomeSeeker} disabled={switching}
          className="w-full flex items-center justify-center gap-1.5 border border-border text-foreground/90 text-xs font-medium py-2 rounded-md hover:border-foreground/40 disabled:opacity-50 transition-colors">
        ...
        <div className="space-y-2">
          <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company name"
            className="w-full border border-border rounded-md py-2 px-3 text-base sm:text-sm text-foreground placeholder:text-muted-foreground focus:border-foreground focus:outline-none transition-colors" />
          <div className="flex gap-2">
            <button onClick={() => setShowForm(false)} className="flex-1 border border-border text-foreground/80 text-xs font-medium py-2 rounded-md hover:border-foreground/40 transition-colors">Cancel</button>
            <button onClick={becomeRecruiter} disabled={switching}
              className="flex-1 flex items-center justify-center gap-1.5 bg-primary text-primary-foreground text-xs font-medium py-2 rounded-md hover:opacity-90 disabled:opacity-50 transition-colors">
        ...
        <button onClick={() => setShowForm(true)}
          className="w-full flex items-center justify-center gap-1.5 border border-border text-foreground/90 text-xs font-medium py-2 rounded-md hover:border-foreground/40 transition-colors">
