...
export default function PostingDetailPage() {
  ...
  if (loading) return <div className="flex items-center justify-center min-h-[400px]"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div>
  if (!posting) return <div className="text-center py-16 text-sm text-muted-foreground">Posting not found. <Link href="/hire" className="text-foreground underline">Back to hiring</Link></div>

  return (
    <div className="space-y-6">
      <Link href="/hire" className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors">
        <ArrowLeft className="w-3.5 h-3.5" /> Back
      </Link>

      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 border-b border-border/60 pb-5">
        <div className="min-w-0">
          ...
          {posting.skills.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {posting.skills.map((s, i) => <span key={i} className="bg-accent text-[10px] text-foreground/80 px-2 py-0.5 rounded">{s}</span>)}
            </div>
          )}
        </div>
        <button onClick={findCandidates} disabled={scanning}
          className="flex items-center justify-center gap-1.5 bg-primary text-primary-foreground text-xs font-medium px-3 py-2 rounded-md hover:opacity-90 disabled:opacity-50 transition-colors shrink-0">
      ...
      <div className="bg-card border border-border rounded-lg p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className="text-xs font-semibold text-foreground tabular-nums w-9">{minScore}%</span>
        </div>
        <input type="range" min="0" max="100" step="5" value={minScore}
          onChange={(e) => setThreshold(Number(e.target.value))}
          className="w-full sm:flex-1 h-1 bg-muted rounded appearance-none cursor-pointer accent-foreground" />
      ...
      {candidates.length === 0 ? (
        <div className="text-center py-14 border border-dashed border-border rounded-lg">
          <Users className="w-6 h-6 text-muted-foreground/60 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">No candidates scored yet</p>
          <p className="text-xs text-muted-foreground mt-1 max-w-sm mx-auto leading-relaxed">
            Click <span className="font-medium text-foreground/80">Find candidates</span> to scan the opt-in talent pool and rank matches for this role.
          </p>
        </div>
      ) : visibleCandidates.length === 0 ? (
        <div className="text-center py-14 border border-dashed border-border rounded-lg">
          <Users className="w-6 h-6 text-muted-foreground/60 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">No candidates at {minScore}%+ match</p>
          <p className="text-xs text-muted-foreground mt-1">Lower the threshold to see more of the {candidates.length} scored candidate{candidates.length === 1 ? "" : "s"}.</p>
        </div>
      ) : (
          ...
          {visibleCandidates.map((c) => (
            <div key={c.id} className="bg-card border border-border rounded-lg p-4">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-sm font-medium text-foreground">{c.candidate?.full_name || "Candidate"}</p>
                ...
                <select value={c.status} onChange={(e) => setStatus(c.id, e.target.value as CandidateStatus)}
                  className="bg-card border border-border text-xs text-foreground/80 rounded-md py-1 px-2 focus:outline-none focus:border-foreground cursor-pointer capitalize shrink-0">
                  {STATUS_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}
                </select>
              </div>

              {c.match_reason && <p className="text-xs text-foreground/80 italic mt-2 leading-relaxed">&quot;{c.match_reason}&quot;</p>}

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                <div>
                  <p className="text-[9px] text-green-700 font-medium uppercase mb-1">Matched</p>
                  <div className="flex flex-wrap gap-1">
                    {c.matched_skills.length === 0 ? <span className="text-[11px] text-muted-foreground italic">None</span>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {c.missing_skills.length === 0 ? <span className="text-[11px] text-muted-foreground italic">None</span>
              ...
              <div className="flex items-center gap-3 mt-3 pt-3 border-t border-border/60">
                {c.candidate?.email && (
                  <a href={`mailto:${c.candidate.email}`} className="inline-flex items-center gap-1.5 text-xs text-foreground/80 hover:text-foreground transition-colors">
                    <Mail className="w-3.5 h-3.5" /> {c.candidate.email}
                  </a>
                )}
                {c.candidate?.linkedin_url && (
                  <a href={c.candidate.linkedin_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-xs text-foreground/80 hover:text-foreground transition-colors">
